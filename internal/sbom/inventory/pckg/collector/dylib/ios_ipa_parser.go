...
func (p *IPAParser) Parse(path string) ([]model.Package, error) {
	...
	workDir, err := os.MkdirTemp(os.TempDir(), "ipa-dylib-*")
	if err != nil {
		return nil, err
	}
	defer func() {
...
func parseDylibFile(file *zip.File, workDir string, sourcePath string) ([]model.Package, error) {
	...
	writer, err := os.CreateTemp(workDir, filepath.Base(file.Name)+".*")
	if err != nil {
		return nil, err
	}
	tempFile := writer.Name()
	defer func() {
		_ = writer.Close()
	...
	for i := 0; i < len(fatFile.Arches); i++ {
		...
		if e != nil {
			log.Warnf("parse macho file error: %s", e.Error())
			continue
		}
		pkgs = append(pkgs, p...)
	}
...
func parseDylibLine(line string, sourcePath string) *model.Package {
	...
	version = normalizeDylibVersion(version)
...
func normalizeDylibVersion(version string) string {
