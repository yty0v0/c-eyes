...
func parsePomFile(path string) ([]model.Package, error) {
	...
	if pomInfo.Dependencies == nil {
		return pkgs, nil
	}
	for _, dependencyPkg := range *pomInfo.Dependencies {
		groupId := trim(stringValue(dependencyPkg.GroupID))
		artifactId := trim(stringValue(dependencyPkg.ArtifactID))
		version := trim(stringValue(dependencyPkg.Version))
