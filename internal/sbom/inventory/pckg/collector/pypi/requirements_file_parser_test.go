...
func TestParseRequirementsFileWithUTF8BOM(t *testing.T) {
	tempDir := t.TempDir()
	path := filepath.Join(tempDir, "requirements-bom.txt")
	content := "\uFEFFrequests==2.31.0\nflask==3.0.3\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatalf("write temp requirements file failed: %v", err)
	}

	parse := NewRequirementsParser()
	pkgs, err := parse.Parse(path)
	if err != nil {
		t.Fatalf("parse requirements with BOM failed: %v", err)
	}
	if len(pkgs) != 2 {
	...
	if pkgs[1].Name != "flask" || pkgs[1].Version != "3.0.3" {
		t.Fatalf("unexpected second package: %#v", pkgs[1])
	}
	for _, pkg := range pkgs {
		if strings.ContainsRune(pkg.Name, '\uFEFF') {
			t.Fatalf("package name still contains BOM rune: %q", pkg.Name)
		}
	}
}
