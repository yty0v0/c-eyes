...
var (
	...
	regPunctuation            = regexp.MustCompile(`[-鈥掆€撯€斺€曗亾飧衡富~藯鈥愨€戔亙鈦烩倠_鈭掆埣鈳彜鈹€鉃栶悊懼婏箻锕ｏ紞]+`)
...
func ParseLicenseFromDir(path string) (value []string, other string, exists bool) {
	...
	if err != nil {
		log.Errorf("涓诲寘license鎵弿璺緞澶辫触 %q: %v\n", path, err)
	}
