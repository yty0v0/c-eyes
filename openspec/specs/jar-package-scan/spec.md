# jar-package-scan Specification

## Purpose
Collect and normalize jar package metadata across Windows and Linux endpoints for inventory and export workflows.

## Requirements
### Requirement: CLI MUST provide jar package collection command
The system SHALL provide an `c-eyes jar-package-scan` command to collect jar package metadata from Windows and Linux endpoints and return normalized records.

#### Scenario: Default execution returns records
- **WHEN** the user runs `c-eyes jar-package-scan` without optional filters
