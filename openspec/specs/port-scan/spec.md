...
定义端口信息采集能力的行为边界、过滤规则与跨平台输出契约，确保 `c-eyes port-scan` 在 Linux 与 Windows 上以一致结构返回端口资产数据，并明确该能力仅用于信息收集而不包含风险分析。

## Requirements

### Requirement: CLI 提供端口扫描命令
系统 SHALL 提供 `c-eyes port-scan` 命令用于执行本机端口信息采集，并返回结构化结果。

#### Scenario: 默认执行扫描
- **WHEN** 用户执行 `c-eyes port-scan` 且未指定扫描模式
...
系统 SHALL 以固定字段集合返回扫描结果，字段包含 `displayIp`、`externalIpList`、`internalIpList`、`bizGroupId`、`bizGroup`、`remark`、`hostTagList`、`proto`、`port`、`pid`、`processName`、`bindIp`、`status`。
