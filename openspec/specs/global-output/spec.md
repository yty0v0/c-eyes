...
定义全局输出参数 `-o/--output` 的格式识别、默认导出行为，以及其在扫描与风险分析链路中的统一输出契约。
## Requirements
...
#### Scenario: 默认导出首个文件名
- **WHEN** 用户执行 `c-eyes hostscan` 且未提供 `-o`
...
- **WHEN** 用户执行 `c-eyes filescan -r -o analysis.csv`
- **THEN** 系统将风险分析结果以 CSV 形式写入 `analysis.csv`

#### Scenario: 独立风险分析导出 Excel
- **WHEN** 用户执行 `c-eyes -r -file hosts.json -o risk.xlsx`
