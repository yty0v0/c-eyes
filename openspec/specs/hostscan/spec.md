...
系统 SHALL 提供 `c-eyes hostscan` 作为主机信息扫描唯一外部入口，支持 `--custom` 与 `--all` 模块选择。

#### Scenario: 默认执行 all 扫描
- **WHEN** 用户执行 `c-eyes hostscan` 且未提供 `--custom/--all`
- **THEN** 系统以 `all` 选择执行全部 host 模块并返回聚合结果
...
- **WHEN** 用户执行 `c-eyes hostscan --custom account` 并提供 account 专属参数
- **THEN** 系统接受请求并按 account 模块参数规则执行

#### Scenario: 多模块使用非交集参数
- **WHEN** 用户执行 `c-eyes hostscan --custom account,process` 并提供仅 account 支持的参数
...
- **WHEN** 用户在 `hostscan -r` 场景设置 `local_only` 风险模式
...
- **WHEN** 用户执行 `c-eyes hostscan --custom application -r` 且扫描结果为 0 条
- **THEN** 系统返回空分析数据 `[]`，并以退出码 0 结束
