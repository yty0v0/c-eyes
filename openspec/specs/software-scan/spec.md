# software-scan Specification

## Purpose
Define the software inventory collection capability used by the `filescan` software module in web mode.

## Requirements
### Requirement: Filescan SHALL provide a software module for software inventory collection
The system SHALL support `software` as a selectable `filescan` web-mode module, available through `c-eyes filescan --custom software` and included in `c-eyes filescan --all`.
